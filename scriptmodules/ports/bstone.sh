...
function build_bstone() {
    mkdir build
    cmake . -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="$md_build/build/install"
    cmake --build . --target install
	md_ret_require="$md_build/build/install/bstone"
}

function install_bstone() {
	md_ret_files=(
        'build/install/bstone'
	)
}
...
function configure_bstone() {
    ...
    mkUserDir $home/.local/share/bibendovsky
    mkUserDir $home/.local/share/bibendovsky/bstone
