...
function configure_lwgmenager() {
    ...
    # temporary directory used for uploads (was hardcoded to /home/pi/tmp upstream)
    local tmp_dir="$home/tmp"
    mkUserDir "$tmp_dir"
    chmod 1777 "$tmp_dir"

    # patch upstream hardcoded /home/pi paths to work for the current RetroPie user
    ...
    # pick the correct config.txt location for the running OS:
    #   Buster (32-bit)            -> /boot/config.txt          -> "32"
    #   Bullseye / Trixie (64-bit) -> /boot/firmware/config.txt -> "64"
    local config_location="32"
    [[ -f /boot/firmware/config.txt ]] && config_location="64"

    # (re)generate the persistent config so the config.txt option matches this OS
    ...
    # install a systemd service pointing at this installation. It runs as root so
    # the web panel can reboot/shutdown the Pi and edit config.txt.
